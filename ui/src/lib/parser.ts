 ...
 * @param code
 */
export function parsePython_old(code: string): ParseResult {
...
export function parsePython(code: string): ParseResult {
  ...
  const class_def = "(module (class_definition (identifier) @function))";
  const vardef = `
     (module (expression_statement (assignment left: (identifier) @vardef)))
     (module (expression_statement (assignment left: (pattern_list (identifier) @vardef))))
   `;
  ...
  query_func.matches(tree.rootNode).forEach((match) => {
    ...
    if (visited.has(JSON.stringify([node.startIndex, node.endIndex]))) return;
    visited.add(JSON.stringify([node.startIndex, node.endIndex]));
    if (match.captures[0].name === "skip") return;
    annotations.push({
      name: node.text, // the name of the function or variable
      // FIXME the name may not be "callsite".
      type: match.captures[0].name as
  ...
  const varuse = findUndefinedSymbols(tree);
  // console.log("varuse:", varuse);
  annotations = annotations.concat(varuse.annotations);
 ...
 */
export function findUndefinedSymbols(tree: Parser.Tree): ParseResult {
  let annotations: Annotation[] = [];
